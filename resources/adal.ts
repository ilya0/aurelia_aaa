...
export class AuthenticationContext {
    ...
    private _addAdalFrame(iframeId: string): any {
        if (typeof iframeId === "undefined") {
            return;
        }

        this.info(`Add adal frame to document:${iframeId}`);
        let adalFrame: any = <HTMLIFrameElement>document.getElementById(iframeId);
    ...
    public getCachedToken(resource: string): null | string {
        ...
        if (expired && (Number(expired) > this._now() + offset)) {
            return token;
        } else {
